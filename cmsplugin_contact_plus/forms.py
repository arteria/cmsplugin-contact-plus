...
class ContactFormPlus(forms.Form):
    required_css_class = getattr(
        settings, 'CONTACT_PLUS_REQUIRED_CSS_CLASS', 'required')
        ...
        if 'instance' not in kwargs:
            for extraField in contactFormInstance.extrafield_set.all():
                if extraField.fieldType == 'CharField':
                    ...
                    if request:
                        lInitial = request.GET.get(
                            slugify(extraField.label), 'n/a')
                    self.fields[slugify(extraField.label)] = forms.CharField(label=extraField.label,
                                                                             # NOTE:
                                                                             # This
                                                                             # overwrites
    ...
    def send(self, recipient_email, request, ts, instance=None, multipart=False):
        ...
        if instance:
            order = ContactPlus.objects.get(
                id=instance.id).extrafield_set.order_by('inline_ordering_position')
        ...
        tmp_headers = {}
        try:
            reply_email_label = getattr(
                settings, 'CONTACT_PLUS_REPLY_EMAIL_LABEL', None)
            if reply_email_label is not None:
                tmp_headers.update(
                    {'Reply-To': self.cleaned_data[reply_email_label]})
        except:
            pass

        send_email(**{
            'from_email': getattr(settings, 'CONTACT_PLUS_FROM_EMAIL',
        ...
        if instance.collect_records:  # and not multipart:
            ...
            record = ContactRecord(
                contact_form=instance, data=ordered_dic_list)
            record.save()
